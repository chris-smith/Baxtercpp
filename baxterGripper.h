...
void BaxterGripper::open() {
    // open gripper to max position
    this->go_to(100);
}

void BaxterGripper::close() {
    // close gripper to max position
    this->go_to(0);
}

void BaxterGripper::clear_calibration() {
    // clear gripper calibration, only for electric grippers
    if (this->type() != "electric")
        return;
    
    // make sure to wait for this to finish
...
void BaxterGripper::calibrate() {
    // calibrate gripper, only for electric grippers
    if (this->type() != "electric")
        return;
...
void BaxterGripper::go_to(float pos) {
    ...
    // argument is json position [0-100] : [closed-open]
